...
def scrap_page(targetURL):
    ...
    with open(f"csv/{name}.csv", "w+", encoding="utf8") as card:
        ...
        if req.status_code == 200:
            ...
            # Extract sentences
            div = html.find("div", class_="post-content")  # Sentences are under this div
            ...
            audiosURLs = list(map(lambda url: f"https://blog.influx.com.br{url}", audiosURLs))  # Add domain to audio URLs
            portugueseSentences = list(map(format_portuguese_sentence, portugueseSentences))  # Clean and format Portuguese
            englishSentences = list(map(format_english_sentence, englishSentences))  # Clean and format English

            if (portugueseSentences != englishSentences != audiosURLs):
...
- in '{name}':
    ({len(englishSentences)} english sentences, {len(portugueseSentences)} portuguese sentences, {len(audiosURLs)} audio files.)""")
            ...
            for sentence in cardInfos:
                card.write(f"{sentence[0]}\t{sentence[1]}\t[sound:{sentence[2]}.mp3]\tenglish_influx\n")  # Use TAB as separator
        else:
